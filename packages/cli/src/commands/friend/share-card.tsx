...
export const description = "发送私聊链接分享卡片";
...
export default function FriendShareCard({ args: [uid, url, title], options: { image, content } }: Props) {
  return (
    <IpcMutate
      action={Actions.SEND_CONTACT_SHARE}
      params={{ user_id: uid, url, title, image, content }}
      loadingText="发送分享…"
      successText={`分享已发送到好友 ${uid}`}
    />
  );
}
